...
namespace sacral20230904.Repository
{
    public class DocumentVerificationModelRepository : IDocumentVerificationModelService
    {
        private readonly string connectionString;

        public DocumentVerificationModelRepository(string connectionString)
        {
        ...
        public async Task<DocumentVerificationModel> GetById(int id)
        {
            using (var connection = new NpgsqlConnection(connectionString))
            {
                ...
                using (var command = new NpgsqlCommand("SELECT * FROM document_verification WHERE id = @id", connection))
                {
        ...
        public async Task<List<DocumentVerificationModel>> GetAll()
        {
            ...
            using (var connection = new NpgsqlConnection(connectionString))
            {
                ...
                using (var command = new NpgsqlCommand("SELECT * FROM document_verification", connection))
                {
                    using (var reader = await command.ExecuteReaderAsync())
                    {
        ...
        public async Task<bool> Delete(int id)
        {
            using (var connection = new NpgsqlConnection(connectionString))
            {
                ...
                using (var command = new NpgsqlCommand("DELETE FROM document_verification WHERE id = @id", connection))
                {
                    command.Parameters.AddWithValue("id", id);
